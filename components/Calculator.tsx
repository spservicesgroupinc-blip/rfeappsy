        ...
        <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in zoom-in duration-200 pb-24">
                        ...
                        <div className="text-lg font-bold text-white flex items-center gap-2">
                            {results.totalCost > 0 ? (
                                <>
                                    ...
                                    <span className={`text-[10px] uppercase px-2 py-0.5 rounded ${(results.totalCost - (results.materialCost + results.laborCost + results.miscExpenses)) / results.totalCost > 0.3
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="bg-slate-100 p-1 rounded-lg flex items-center">
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-md text-[9px] font-black uppercase tracking-wider transition-all ${state.pricingMode === 'level_pricing' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600'
                                    }`}
                            >
                                Cost Plus
                            </button>
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-md text-[9px] font-black uppercase tracking-wider transition-all ${state.pricingMode === 'sqft_pricing' ? 'bg-white shadow-sm text-brand' : 'text-slate-400 hover:text-slate-600'
                                    }`}
                            >
                                SqFt Price
                            </button>
                ...
                <div className="md:col-span-2 flex flex-col md:flex-row gap-4 pt-6 border-t border-slate-200 pb-12">
                    ...
                    {/* Conditional Workflow Buttons - STRICT LINEAR FLOW */}
                    ...
                    {currentStatus === 'Paid' ? (
                        ...
                        // 2. INVOICED -> Show Payment Action
                        <button
                            onClick={onStageInvoice}
                            className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white p-4 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-emerald-200"
                        >
                            <CheckCircle2 className="w-4 h-4" /> Record Payment
                        </button>
                    ) : currentStatus === 'Work Order' ? (
                        // 3. WORK ORDER -> Schedule OR Invoice
                        // Linear: Must Schedule first. If Scheduled, then can Invoice.
                        !activeScheduledDate ? (
                            <button onClick={onStageWorkOrder} className="flex-1 bg-amber-500 hover:bg-amber-600 text-white p-4 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-amber-200">
                                <Calendar className="w-4 h-4" /> Dispatch & Schedule
                            </button>
                        ) : (
                            // If scheduled, show invoice options. User can still edit WO if needed, but primary push is forward.
                            <div className="flex-1 flex gap-2">
                                <button onClick={onStageWorkOrder} className="px-6 bg-white border-2 border-slate-100 hover:bg-slate-50 text-slate-400 p-4 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2">
                                ...
                                <button onClick={onStageInvoice} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white p-4 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-emerald-200">
                                    <Receipt className="w-4 h-4" /> Finalize & Invoice
                                </button>
                            </div>
                        )
                    ) : (
                        // 4. DRAFT -> Move to Sold
                        // This is default for Draft if not using "Review & Finalize"
                        <div className="flex-1 flex items-center justify-center text-xs text-slate-400 font-bold italic">
                            Finalize Estimate to proceed
                        </div>
                    )}
                </div>
            </div>
