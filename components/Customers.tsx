...
import { Plus, Archive, Phone, Mail, MapPin, ArrowLeft, FileText, ClipboardList, HardHat, Receipt, CheckCircle2 } from 'lucide-react';
        ...
        return (
            <div className="space-y-6 animate-in fade-in zoom-in duration-200">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-6 border-b border-slate-100 bg-slate-50 font-black uppercase text-[10px] tracking-widest text-slate-400">Project Documents & History</div>
                            ...
                            <tbody>
                                {customerEstimates.map(est => (
                                    <tr key={est.id} className="hover:bg-slate-50 border-b last:border-0 cursor-pointer transition-colors" onClick={() => onLoadEstimate(est)}>
                                        ...
                                        <td className="px-6 py-5"> <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${est.status === 'Paid' ? 'bg-emerald-100 text-emerald-700' :
                                ...
                                {customerEstimates.length === 0 && <tr><td colSpan={4} className="p-12 text-center text-slate-300 italic">No job history found.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </div>
