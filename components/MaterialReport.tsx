...
export const MaterialReport: React.FC<MaterialReportProps> = ({ state, onBack }) => {
    ...
    const filteredLogs = useMemo(() => {
        return (state.materialLogs || []).filter(log => log && log.date && typeof log.date === 'string' && log.date.startsWith(filterMonth));
                        ...
                        {filteredLogs.length === 0 ? (
                            ...
                            filteredLogs.map((log, idx) => (
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    <td className="px-6 py-4 font-medium text-slate-500">{log.date ? new Date(log.date).toLocaleDateString() : 'N/A'}</td>
