...
import { deleteEstimate, markJobPaid, createWorkOrderSheet, syncUp, savePdfToDrive } from '../services/api';
import { generateWorkOrderPDF, generateDocumentPDF, generateDocumentBase64, generateWorkOrderBase64, generateCompletionReportBase64 } from '../utils/pdfGenerator';
...
export const useEstimates = () => {
    ...
    // Helper to Generate & Upload PDF
    const generateAndUploadPDF = async (record: EstimateRecord, type: 'ESTIMATE' | 'WORK_ORDER' | 'INVOICE' | 'RECEIPT' | 'COMPLETION_REPORT', silent: boolean = false) => {
        if (!session?.spreadsheetId) return null;

        if (!silent) dispatch({ type: 'SET_NOTIFICATION', payload: { type: 'success', message: `Generating ${type === 'COMPLETION_REPORT' ? 'Report' : type}...` } });
        ...
        try {
            ...
            if (type === 'WORK_ORDER') {
                const res = await generateWorkOrderBase64(appData, record);
                base64Data = res.base64;
                fileName = res.filename;
            } else if (type === 'COMPLETION_REPORT') {
                const res = await generateCompletionReportBase64(appData, record);
                base64Data = res.base64;
                fileName = res.filename;
            } else {
                const res = await generateDocumentBase64(appData, record.results, type as any, record);
                base64Data = res.base64;
                fileName = res.filename;
            }

            if (!silent) dispatch({ type: 'SET_NOTIFICATION', payload: { type: 'success', message: 'Uploading to Cloud...' } });
            const url = await savePdfToDrive(fileName, base64Data, record.id, session.spreadsheetId, session.folderId);

            if (url) {
                let updatedRecord = { ...record };

                // Update local record based on type
                if (type === 'INVOICE') updatedRecord.invoicePdfLink = url;
                else if (type === 'COMPLETION_REPORT') updatedRecord.completionReportLink = url;
                else updatedRecord.pdfLink = url; // Updates main link for Estimate/WorkOrder/Receipt

                // Update in state
                const updatedEstimates = appData.savedEstimates.map(e => e.id === record.id ? updatedRecord : e);
                dispatch({ type: 'UPDATE_DATA', payload: { savedEstimates: updatedEstimates } });

                if (!silent) dispatch({ type: 'SET_NOTIFICATION', payload: { type: 'success', message: 'Document Saved to Cloud!' } });
                return updatedRecord;
            }
        } catch (e) {
            console.error("PDF Upload Error", e);
            if (!silent) dispatch({ type: 'SET_NOTIFICATION', payload: { type: 'error', message: 'Cloud Save Failed' } });
        }
        return null;
    };
    ...
    const saveEstimate = async (results: CalculationResults, targetStatus?: EstimateRecord['status'], extraData?: Partial<EstimateRecord>, shouldRedirect: boolean = true) => {
        ...
        const newEstimate: EstimateRecord = {
            ...
            pdfLink: existingRecord?.pdfLink,
            invoicePdfLink: existingRecord?.invoicePdfLink,
            completionReportLink: existingRecord?.completionReportLink,
            sitePhotos: existingRecord?.sitePhotos,
    ...
    const handleMarkPaid = async (id: string) => {
        ...
        if (estimate) {
            ...
            if (result.success && result.estimate) {
                ...
                await generateAndUploadPDF(result.estimate!, 'RECEIPT', true);
            } else {
    ...
    const handleBackgroundWorkOrderGeneration = async (record: EstimateRecord, currentWarehouse: any) => {
        ...
        try {
            // A. Generate & Upload Work Order PDF (NEW)
            const pdfRecord = await generateAndUploadPDF(record, 'WORK_ORDER', true);
            const currentRecord = pdfRecord || record;

            // B. Create Standalone Sheet for Crew Log (Slow API Call)
            const woUrl = await createWorkOrderSheet(currentRecord, session.folderId, session.spreadsheetId);
